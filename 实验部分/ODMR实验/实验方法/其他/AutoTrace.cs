...
namespace ODMR_Lab.实验部分.ODMR实验.实验方法.其他
{
    ...
    class AutoTrace : ODMRExperimentWithoutAFM
    {
        ...
        public List<object> FirstScanEvent(NanoStageInfo device, D1NumricScanRangeBase range, double locvalue, List<object> inputParams)
        {
        ...
        public List<object> ScanEvent(NanoStageInfo device, D1NumricScanRangeBase range, double locvalue, List<object> inputParams)
        {
            //移动位移台
        ...
        public override void ODMRExpWithoutAFM()
        {
            ...
            session.BeginScan(new D1NumricLinearScanRange(pos - range / 2, pos + range / 2, count), 0, 100.0 / 3);
            //拟合
            ...
            session.BeginScan(new D1NumricLinearScanRange(pos - range / 2, pos + range / 2, count), 100.0 / 3, 2 * 100.0 / 3);
            //拟合
          ...
          new NumricDataSeries("高斯拟合(Y)", fitx, fity) { LineColor = Colors.LightSkyBlue }));
            ...
            session.BeginScan(new D1NumricLinearScanRange(pos - range / 2, pos + range / 2, count), 2 * 100.0 / 3, 100);
            //拟合
