        ...
        public static void ScanCW(ODMRExpObject parentexp, int CWExpIndex, out List<double> peaks, out List<double> fitcontrasts, out List<double> freqvalues, out List<double> contractvalues, double peakapprox, int EndPointCount, double scanWidth = 30, bool? isReverse = null)
        {
            ...
            if (freqs.Count != 0)
            {
                ...
                if (Math.Abs(f1 - peakapprox + scanWidth) < 3)
                {
                    ...
                    {
                        //细扫
                        CW(parentexp, CWExpIndex, out freqvalues, out contractvalues, out List<double> f1s, out fitcontracts, startFreq: freqs[0] - 5, endFreq: freqs[0] + 5, step: 1, averagetime: 2000, peakcount: 1, EndPointCount: 8, isReverse: isReverse);
                        if (f1s.Count != 0)
                        {
                            peaks.Add(f1s[0]);
                ...
                {
                    CW(parentexp, CWExpIndex, out freqvalues, out contractvalues, out List<double> f1s, out fitcontracts, startFreq: f1 - 5, endFreq: f1 + 5, step: 1, averagetime: 2000, peakcount: 1, EndPointCount: 8, isReverse: isReverse);
                    if (f1s.Count != 0)
                    {
                        peaks.Add(f1s[0]);
                        fitcontrasts.Add(fitcontracts[0]);
