...
namespace ODMR_Lab.实验部分.ODMR实验.实验方法.其他
{
    ...
    public partial class MagnetLoc : ODMRExperimentWithoutAFM
    {
        ...
        protected void ScanCW(out List<double> peaks, out List<double> freqvalues, out List<double> contractvalues, double peakapprox, int EndPointCount, double scanWidth = 30, bool isReverse = false)
        {
            ...
            peaks = new List<double>();
            CW(out freqvalues, out contractvalues, out List<double> freqs, out List<double> fitcontracts, startFreq: peakapprox - scanWidth, endFreq: peakapprox + scanWidth, step: 2, averagetime: 500, peakcount: 1, EndPointCount, isReverse: isReverse);
            ...
            if (freqs.Count != 0)
            {
                ...
                {
                    CW(out freqvalues, out contractvalues, out freqs, out fitcontracts, startFreq: peakapprox - scanWidth - 20, endFreq: peakapprox - scanWidth + 20, step: 2, averagetime: 500, peakcount: 1, EndPointCount, isReverse: isReverse);
                    if (freqs.Count != 0)
                    {
                        //细扫
                        CW(out freqvalues, out contractvalues, out List<double> f1s, out fitcontracts, startFreq: freqs[0] - 5, endFreq: freqs[0] + 5, step: 1, averagetime: 2000, peakcount: 1, EndPointCount: 8, isReverse: isReverse);
                        if (f1s.Count != 0)
                        {
                            peaks.Add(f1s[0]);
                        }
                ...
                if (Math.Abs(f1 - peakapprox - scanWidth) < 3)
                {
                    ...
                    if (freqs.Count != 0)
                    {
                        CW(out freqvalues, out fitcontracts, out List<double> f1s, out fitcontracts, startFreq: freqs[0] - 5, endFreq: freqs[0] + 5, step: 1, averagetime: 2000, peakcount: 1, EndPointCount: 8, isReverse: isReverse);
                        if (f1s.Count != 0)
                        {
                            peaks.Add(f1s[0]);
                        }
                ...
                {
                    CW(out freqvalues, out contractvalues, out List<double> f1s, out fitcontracts, startFreq: f1 - 5, endFreq: f1 + 5, step: 1, averagetime: 2000, peakcount: 1, EndPointCount: 8, isReverse: isReverse);
                    if (f1s.Count != 0)
                    {
                        peaks.Add(f1s[0]);
                    }
