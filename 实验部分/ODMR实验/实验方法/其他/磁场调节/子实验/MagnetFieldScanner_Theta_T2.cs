...
namespace ODMR_Lab.实验部分.ODMR实验.实验方法.其他.磁场调节.子实验
{
    ...
    /// 磁场扫描
    /// </summary>
    class MagnetFieldScanner_Theta_T2 : PulseExpBase
    {
        ...
        public override string ODMRExperimentName { get; set; } = "磁场方向遍历扫描(T2,扫描变量Theta-T2)";
        ...
        public override void ODMRExpWithoutAFM()
        {
            ...
            {
                SetExpState("当前磁场角度  θ:" + Math.Round(p.Y, 5).ToString() + "  T2时间:" + Math.Round(p.X, 5).ToString());
            });
        ...
        public override void PreExpEventWithoutAFM()
        {
            ...
            D2ChartDatas.Add(new ChartData2D(new FormattedDataSeries2D(xcount, xlo, xhi, ycount, ylo, yhi) { XName = "T2时间t", YName = "方位角θ", ZName = "0态对比度" }) { GroupName = "T2数据" });
            D2ChartDatas.Add(new ChartData2D(new FormattedDataSeries2D(xcount, xlo, xhi, ycount, ylo, yhi) { XName = "T2时间t", YName = "方位角θ", ZName = "1态对比度" }) { GroupName = "T2数据" });
            D2ChartDatas.Add(new ChartData2D(new FormattedDataSeries2D(xcount, xlo, xhi, ycount, ylo, yhi) { XName = "T2时间t", YName = "方位角θ", ZName = "0态计数" }) { GroupName = "T2数据" });
            D2ChartDatas.Add(new ChartData2D(new FormattedDataSeries2D(xcount, xlo, xhi, ycount, ylo, yhi) { XName = "T2时间t", YName = "方位角θ", ZName = "1态计数" }) { GroupName = "T2数据" });
            D2ChartDatas.Add(new ChartData2D(new FormattedDataSeries2D(xcount, xlo, xhi, ycount, ylo, yhi) { XName = "T2时间t", YName = "方位角θ", ZName = "参考计数" }) { GroupName = "T2数据" });
            UpdatePlotChart();
        ...
        {
            //移动位移台
            MagnetScanTool.CalculatepPredictLoc(Ps, Ps.ZLoc, arg4.Y, GetInputParamValueByName("PhiLoc"), out double x, out double y, out double z, out double angle);
            ...
            var result = GetT2Data((int)arg4.X, (double)arg5[0]);
