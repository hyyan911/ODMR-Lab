...
namespace ODMR_Lab.实验部分.ODMR实验.实验方法.无AFM.点实验.脉冲实验
{
    class Rabi : PulseExpBase
    {
        ...
        public override void AfterExpEventWithoutAFM()
        {
            ...
            var xs = Get1DChartDataSource("微波驱动时间(ns)", "Rabi对比度数据").Where(x => !double.IsNaN(x)).ToList();
            var ys_x = Get1DChartDataSource("通道X Rabi信号对比度[(sig-ref)/ref]", "Rabi对比度数据").Where(x => !double.IsNaN(x)).ToList();
            ...
            var ys_y = Get1DChartDataSource("通道Y Rabi信号对比度[(sig-ref)/ref]", "Rabi对比度数据").Where(x => !double.IsNaN(x)).ToList(); ;
