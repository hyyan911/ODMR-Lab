...
namespace ODMR_Lab.实验部分.ODMR实验.实验方法.无AFM.点实验.脉冲实验
{
    class Rabi : PulseExpBase
    {
        ...
        public List<object> ScanEvent(object device, D1NumricScanRangeBase range, double locvalue, int currrentloop, List<Tuple<string, string, double, MultiLoopDataProcessBase>> outputparams, List<object> inputParams)
        {
            ...
            PulsePhotonPack pack = DoPulseExp("Rabi", GetInputParamValueByName("RFFrequency"), GetInputParamValueByName("RFAmplitude"), GetInputParamValueByName("SeqLoopCount"), 6, GetInputParamValueByName("TimeOut"));

            //光子数
            double signalcountX = pack.GetPhotonsAtIndex(0).Sum();
            double refcount = pack.GetPhotonsAtIndex(2).Sum();
            double signalcountY = pack.GetPhotonsAtIndex(1).Sum();
            ...
            {
                if (refcount != 0)
                    signalcontrastX = (signalcountX - refcount) / refcount;
            }
            catch (Exception)
            {
            }
            ...
            {
                if (refcount != 0)
                    signalcontrastY = (signalcountY - refcount) / refcount;
            }
            catch (Exception)
            {
            }
            ...
            outputparams.Add(new Tuple<string, string, double, MultiLoopDataProcessBase>("平均光子数", "Rabi荧光数据", refcount, new StandardDataProcess()));
            ...
            outputparams.Add(new Tuple<string, string, double, MultiLoopDataProcessBase>("通道Y 信号光子数", "Rabi荧光数据", signalcountY, new StandardDataProcess()));
        ...
        private void PlotEvent(List<MultiLoopScanData> list)
        {
            ...
            (Get1DChartData("平均光子数", "Rabi荧光数据") as NumricChartData1D).Data = MultiLoopScanData.GetAverageData(list, "平均光子数", "Rabi荧光数据");
            ...
            (Get1DChartData("平均光子数", "方差") as NumricChartData1D).Data = MultiLoopScanData.GetSigmaData(list, "平均光子数", "Rabi荧光数据");
        ...
        public override void AfterExpEventWithoutAFM()
        {
            ...
            //计算平均光子计数
            OutputParams.Add(new Param<double>("平均光子计数", Get1DChartDataSource("平均光子数", "Rabi荧光数据").Where(x => !double.IsNaN(x)).Average(), "X_AverageCount"));
