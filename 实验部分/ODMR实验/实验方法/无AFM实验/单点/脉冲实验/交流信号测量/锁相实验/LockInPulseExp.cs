...
namespace ODMR_Lab.实验部分.ODMR实验.实验方法.无AFM.点实验.脉冲实验
{
    ...
    class LockInPulseExp : LockInExpBase
    {
        ...
        public override List<ParamB> InputParams { get; set; } = new List<ParamB>()
        {
            ...
            new Param<SequenceType>("序列类型",SequenceType.CMPG,"SequenceType"),
        ...
        private List<object> ScanEvent(object device, D1NumricScanRangeBase range, double locvalue, int currrentloop, List<Tuple<string, string, double, MultiLoopDataProcessBase>> outputparams, List<object> inputParams)
        {
            ...
            if (GetInputParamValueByName("SequenceType") == SequenceType.XY4)
            {
                sequenceXName = "XY-4-halfpiX";
                sequenceYName = "XY-4-halfpiY";
            }
            if (GetInputParamValueByName("SequenceType") == SequenceType.XY8)
            {
                sequenceXName = "XY-8-halfpiX";
                sequenceYName = "XY-8-halfpiY";
            }
            if (GetInputParamValueByName("SequenceType") == SequenceType.CMPG)
            {
                sequenceXName = "CMPGX";
                sequenceYName = "CMPGY";
            }
