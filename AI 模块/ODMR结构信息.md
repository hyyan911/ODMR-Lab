...
### 功能说明
通过外部 AI 指令（`start-experiment`）启动实验时，程序内部不会弹出 `PreConfirmProcedure` 中的确认框（如"是否要继续？此操作将清除原先的实验数据"），也不会弹出 `DropConfirm` 中的下针信息确认框。

### 实现原理
- **`ExperimentObject.cs` 行 53**：新增静态字段 `public static volatile bool SkipPreConfirm = false;`
- **`ExperimentObject.cs` 行 700**：`StartEvent` 方法中，`PreConfirmProcedure()` 调用前检查 `if (!SkipPreConfirm)`
- **`AIService.Experiments.cs` 行 234-245**：`start-experiment` 指令在调用 `exp.Start()` 前设置 `SkipPreConfirm = true`，在 `finally` 块中重置为 `false`
- **`VibrationExpBase.cs` 行 69**：`DropConfirm` 方法中，弹框前检查 `if (!SkipPreConfirm)`

### 安全设计
- 该标志仅在 AI 启动实验的短暂时间窗口内为 `true`，由 `finally` 块保证异常时也能重置
- 用户手动点击"开始实验"按钮时，`SkipPreConfirm` 为 `false`，确认框正常弹出
- AFM 类实验在 AI 安全模式下仍需 `confirm=true` 参数，这是 AI 层面的安全确认，与程序内部弹框独立
